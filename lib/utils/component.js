...
function createInput(item, data, form) {
  ...
  var props = _objectSpread({
    form: form,
    readOnly: action === _frSchema.actions.show || action === _frSchema.actions.edit && item.readOnly,
    disabled: action === _frSchema.actions.show || action === _frSchema.actions.edit && item.readOnly,
  ...
  switch (type) {
    ...
    case "Select":
      ...
      item.dict && Object.values(item.dict).forEach(function (dictItem) {
        ...
        if (dictItem.condition && (action === _frSchema.actions.add || action === _frSchema.actions.edit)) {
...
function createForm(column, data, form) {
  ...
  column.forEach(function (item) {
    ...
    if (action === _frSchema.actions.edit && item.editHide && !data[item.dataIndex]) {
      return;
    }
